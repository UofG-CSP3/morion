...
class TestMongoModel(unittest.TestCase):
    ...
    def test_collection(self):
        collection = model.collection(db=self.db)
    ...
    def test_a_insert(self):
        ...
        assert(die.insert(db=self.db))
        die_with_nonexistent_wafer = Die(wafer='aba', name='abc', anode_type='momma', device_type='pappa', size=69.421, pitch=35.23,
                   n_channels=45.133)
        assert(die_with_nonexistent_wafer.insert(db=self.db))

        assert(fabrun_with_nonexistent_wafer.insert(db=self.db))
    ...
    def test_a_find_one(self):
        ...
        wafer1.insert(db=self.db)
        iv.insert(db=self.db)

        assert(Wafer.find_one(db=self.db,query={'name':'aaa'})==wafer1)
        assert(IV.find_one(db=self.db,query={'voltageStep':3.9})==iv)
    ...
    def test_a_find(self):
        ...
        wafer2 = Wafer(name='bbb', production_date=datetime(year=1999, month=1, day=1),
                       ...
                       handle_wafer='sagaku', sheet_resistance=31.31)
        wafer1.insert(db=self.db)
        wafer2.insert(db=self.db)

        wafers_GOLD = Wafer.find(db=self.db, query={'material_type': 'GOLD'})
    ...
    def test_delete_one(self):
        ...
        die3 = Die(wafer='', name='c', anode_type='anode', device_type='device', size=69.421, pitch=35.23,
                   n_channels=45.133)
        die1.insert(db=self.db)
        die2.insert(db=self.db)
        die3.insert(db=self.db)

        assert(Die.delete_one(db=self.db, query={'size':69.421}))
        assert(Die.find(db=self.db, query={'size':69.421})==())
        assert(len(Die.find(db=self.db, query={}))==2)
    ...
    def test_delete_many(self):
        ...
        assert(Die.delete_many(db=self.db, query={'wafer': 'aaa'}))
        assert(Die.find(db=self.db, query={'wafer':'aaa'})==())
        assert(len(Die.find(db=self.db, query={}))==1)
    ...
    def test_find_one_and_delete(self):
        ...
        assert(Wafer.find_one_and_delete(db=self.db, query={'name':'a'})==wafer1)
        assert(Wafer.find_one(db=self.db, query={'name':'aaa'}) is None)


    def test_update(self):
        die = Die(wafer='bbb', name='die', anode_type='anodDD', device_type='deDDvice', size=69.21, pitch=35.23,
                   n_channels=45.133)
        die.insert(db=self.db)
        id = die.id
        die_update = Die(id=id, wafer='bbb', name='updated_die', anode_type='anodDD', device_type='deDDvice', size=69.23, pitch=35.23,
                   n_channels=45.133)

        assert(die_update.update(db=self.db))
        all_dies = Die.find(db=self.db)
    ...
    def test_insert_or_replace(self):
        die = Die(wafer='bbb', name='a', anode_type='anode', device_type='device', size=2.12, pitch=3.13,
                   n_channels=3.25)
        assert (die.insert_or_replace(db=self.db))
        replacement_die = Die(id=die.id, wafer='bbb', name='a', anode_type='anode', device_type='device', size=2.12,
                      pitch=3.13,
                      n_channels=3.15)
        assert (replacement_die.insert_or_replace(db=self.db))
        assert (replacement_die in Die.find(db=self.db))
        assert (die not in Die.find(db=self.db))
     ...
     #   die1 = Die(wafer='aaa', name='a', anode_type='anode', device_type='device', size=2.12, pitch=3.13,
      #            n_channels=2.1111334)
       # die2 = Die(wafer='bbb', name='b', anode_type='anode', device_type='device', size=2.12, pitch=3.13,
        #           n_channels=3.25)
        #print (die2.find_and_replace(db=self.db, query={'name': 'a'}))
        #assert (die1 not in Die.find(db=self.db))
    ...
    def test_delete(self):
        wafer = Wafer(name='aaa', production_date=datetime(year=1999, month=1, day=1),
                       ...
                       depletion_voltage_stdev=3.21, thickness=3.311,
                       handle_wafer='sagaku', sheet_resistance=31.31)
        wafer.insert(db=self.db)

        wafer.delete(db=self.db)
        assert(Wafer.find(db=self.db, query={})==())
